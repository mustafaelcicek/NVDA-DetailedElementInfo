...
`addon/globalPlugins/detailedElementInfo/`. The add-on uses NV Access's SCons build
system (`buildVars.py` + `sconstruct`); build it with `uv sync && uv run scons`.

Rules:

- Standard library and NVDA APIs only — **no third-party / `pip` dependencies**. Keep
  NVDA 2023.1+ compatibility.
- Keep strings translatable (`_()`) and update the English `readme.md` (repo root) and
  the Turkish `addon/doc/tr/readme.md` when features change; the `readme.html` files are
  generated by the build. Don't touch version numbers, the metadata in `buildVars.py`
  (which generates `manifest.ini`), or locale files unless asked.
