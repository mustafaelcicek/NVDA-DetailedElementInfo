...
- The add-on source lives under `addon/globalPlugins/detailedElementInfo/`.
...
- Target compatibility: **NVDA 2023.1 and newer** (set in `buildVars.py`, which
  generates `manifest.ini`).
- Translatable strings go through NVDA's translation system (`_()` and
  ...
  can be translated.
- User documentation is the English `readme.md` at the repo root and the Turkish
  `addon/doc/tr/readme.md`. If you add or change a feature, update **both** so English
  and Turkish stay in sync. The `readme.html` files are generated by the build — never
  edit them by hand.
...
The add-on uses NV Access's SCons build system. All source lives under `addon/`; the
manifest, compiled translations (`.mo`), and HTML docs are **generated** — don't commit
them (they are git-ignored).

- `uv sync` — install the build/lint tooling (SCons, Markdown, Ruff, prek, Pyright).
- `uv run scons` — build `detailedElementInfo-<version>.nvda-addon` in the repo root.
- `uv run scons pot` — regenerate the `detailedElementInfo.pot` translation template.

You need `gettext` (`msgfmt`) on your PATH for translation compilation. On a pushed tag,
GitHub Actions (`.github/workflows/build_addon.yml`) builds and publishes a release
automatically.
...
1. Copy the `addon/globalPlugins/detailedElementInfo/` folder into your NVDA scratchpad
...
3. **Test it in NVDA by hand** (see above) and update the docs (root `readme.md` and
...
- Match the existing code style and structure in `addon/globalPlugins/detailedElementInfo/`.
- Never add `pip` / third-party dependencies. Standard library only.
- Keep user-facing strings translatable (`_()`), and update the English `readme.md` (repo
  root) **and** the Turkish `addon/doc/tr/readme.md` whenever a feature changes. The
  `readme.html` files are generated — don't edit them.
- Don't change version numbers, the add-on metadata in `buildVars.py` (which generates
  `manifest.ini`), or release metadata unless explicitly asked to.
